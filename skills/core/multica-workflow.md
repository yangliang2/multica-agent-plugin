...
| `child_issues` | array | `[]` | Child issue IDs created from an approved breakdown; read by the leader's stop-hook checkpoint for stuck detection and the all-done → result transition (v2.3.0+) |
| `squad_stuck_threshold_minutes` | integer | `120` | A child with no new comment activity for longer than this (server-time comparison) is reported in a `[checkpoint] squad-stuck` comment (v2.3.0+) |
